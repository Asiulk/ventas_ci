...
$(document).ready(function () {
    ...
    $(".btn-view-producto").on("click", function(){
        ...
        html += "<p><strong>Iva: </strong>"+infoproducto[5]+"%</p>"
        html += "<p><strong>Stock: </strong>"+infoproducto[6]+"</p>"
        html += "<p><strong>Categoria: </strong>"+infoproducto[7]+"</p>"
        iva = (infoproducto[4]*(infoproducto[5]/100))
        total = parseInt(infoproducto[4])+parseInt(iva)
    ...
    $("#producto").autocomplete({
        ...
        select:function(event, ui){
            data = ui.item.id + "*"+ ui.item.codigo+ "*"+ ui.item.label+ "*"+ ui.item.precio+ "*"+ ui.item.iva+ "*"+ ui.item.stock;
    ...
    $("#btn-agregar").on("click",function(){
        ...
        if (data !='') {
            ...
            ivap = (infoproducto[3]*(infoproducto[4]/100))
            totalp = parseInt(infoproducto[3])+parseInt(ivap)
            html += "<td><input type='hidden' name='ivas[]' value='"+ivap+"'>"+ivap+"</td>";
            html += "<td>"+infoproducto[5]+"</td>";
            html += "<td><input type='text' name='cantidades[]' value='1' class='cantidades'></td>";
            html += "<td><input type='hidden' name='preciototal[]' value='"+infoproducto[3]+"'><p>"+infoproducto[3]+"</p></td>";
            html += "<td><input type='hidden' name='ivatotal[]' value='"+ivap+"'><p>"+ivap+"</p></td>";
            html += "<td><input type='hidden' name='importes[]' value='"+totalp+"'><p>"+totalp+"</p></td>";
    ...
    $(document).on("keyup","#tbventas input.cantidades", function(){
        ...
        iva = $(this).closest("tr").find("td:eq(3)").text();
        preciot = parseInt(cantidad) * parseInt(precio);
        ivat = parseInt(cantidad) * parseInt(iva);
        importe = parseInt(preciot) + parseInt(ivat);

        $(this).closest("tr").find("td:eq(6)").children("p").text(preciot);
        $(this).closest("tr").find("td:eq(6)").children("input").val(preciot);
        $(this).closest("tr").find("td:eq(7)").children("p").text(ivat);
        $(this).closest("tr").find("td:eq(7)").children("input").val(ivat);
        $(this).closest("tr").find("td:eq(8)").children("p").text(importe);
        $(this).closest("tr").find("td:eq(8)").children("input").val(importe);
        sumar();
    });
...
function sumar(){
    ...
    $("#tbventas tbody tr").each(function(){
        ...
        igv = igv + Number($(this).find("td:eq(7)").text());
    });
    ...
    function graficar(meses,montos,year){
      Highcharts.chart('grafico', {
        ...
        tooltip: {
            ...
            pointFormat: '<tr><td style="color:red;padding:0">Total:  </td>' +
